-- Migration: Fix auto_cancel_config cron permissions
-- Description: Grant necessary permissions to manage_auto_cancel_cron_job function for cron access
-- Date: 2026-01-06

-- Grant USAGE on cron schema to the function owner (postgres)
GRANT USAGE ON SCHEMA cron TO postgres;

-- Grant necessary permissions on cron tables
GRANT SELECT, INSERT, UPDATE, DELETE ON cron.job TO postgres;

-- Ensure the function has the right permissions by recreating it
-- This version will be explicitly marked as SECURITY DEFINER to run as the owner
