-- ============================================================================
-- Migration: Simplify Auto-Cleanup to call Edge Function directly
-- ============================================================================
-- Instead of calling via net.http_post, we'll call the deno function directly
-- by invoking it as a worker within postgres
-- ============================================================================
...
BEGIN
  ...
  RAISE NOTICE 'Auto-Cleanup hourly check: BRT hour = %, Today = %', 
  ...
  LOOP
    ...
    IF v_current_hour_brt = v_config_hour AND 
      ...
      RAISE NOTICE '[Auto-Cleanup] TRIGGER user=%, hour=%, last_run=%', 
        v_config.user_id, v_config_hour, v_last_run_date;

      -- Get auth token for this user
      SELECT value INTO v_auth_token
      FROM app_config
      WHERE user_id = v_config.user_id
      AND key IN ('auth_token', 'superlogica_refresh_token')
      LIMIT 1;
      ...
      BEGIN
        ...
        RAISE NOTICE '[Auto-Cleanup] HTTP request sent for user %', v_config.user_id;

      EXCEPTION WHEN OTHERS THEN
        RAISE WARNING '[Auto-Cleanup] HTTP failed for user %: %', v_config.user_id, SQLERRM;
      END;
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'auto-cleanup-hourly-check') THEN
    PERFORM cron.schedule(
      'auto-cleanup-hourly-check',
      '0 * * * *',
      'SELECT run_automatic_cleanup();'
    );
    RAISE NOTICE 'Created auto-cleanup cron job';
  END IF;
END $$;
