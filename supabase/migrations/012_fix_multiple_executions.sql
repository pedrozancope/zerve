-- ============================================
-- Migration 012: Corrigir Timezone e Múltiplas Execuções
-- ============================================
-- 
-- Problemas corrigidos:
-- 1. Adiciona coluna last_executed_at para evitar múltiplas execuções
-- 2. Atualiza função para verificar se já foi executado recentemente
-- 3. Desativa schedules 'once' expirados automaticamente
-- 4. Verifica execution_logs antes de re-executar
-- ============================================

-- 1. Adicionar coluna para rastrear última execução
ALTER TABLE schedules 
ADD COLUMN IF NOT EXISTS last_executed_at TIMESTAMPTZ;

COMMENT ON COLUMN schedules.last_executed_at IS 
  'Data/hora da última execução. Usado para evitar múltiplas execuções na janela de tempo.';

-- 2. Atualizar função para evitar múltiplas execuções
...
BEGIN
  ...
  -- Inclui verificação de last_executed_at para evitar duplicatas
  -- ============================================
  FOR v_schedule IN
    SELECT 
      s.id, 
  ...
  LOOP
    BEGIN
      -- ============================================
      -- Verificação extra: checar se já existe log de execução recente
      -- para este schedule (proteção contra falha da Edge Function)
      -- ============================================
      SELECT EXISTS(
        SELECT 1 FROM execution_logs 
        WHERE schedule_id = v_schedule.id 
          AND executed_at > v_now - INTERVAL '10 minutes'
          AND status IN ('success', 'pending')
      ) INTO v_has_recent_log;
      
      IF v_has_recent_log THEN
        RAISE NOTICE 'Schedule % já tem log recente, pulando...', v_schedule.id;
        v_results := v_results || jsonb_build_object(
          'schedule_id', v_schedule.id,
          'schedule_name', v_schedule.name,
          'skipped', true,
          'reason', 'already_has_recent_log'
        );
        CONTINUE;
      END IF;
...
COMMENT ON FUNCTION check_and_execute_schedules() IS 
  'Verifica e executa schedules pendentes. Inclui proteção contra múltiplas execuções e limpeza de schedules expirados.';
